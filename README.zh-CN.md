...
> `brew install video-compare` / `yay -S video-compare` 都能跑,但**没有** v1.0~v1.5 的任何改动 ——
...
| 预览中 `4` `6` `R` | 缩放到 1:1 物理像素 / 100%(一个源像素一个窗口点)/ 适应窗口;当前倍率显示在底部提示条 |
| `F` | 保存两侧当前帧和屏幕内容为 PNG |
...
**`-d` 在 1x 档位(4K 原生)帮倒忙** —— 见已知问题表 `display.cpp:323`。那一档要 1:1 就别加 `-d`。
...
- **此前只能靠滚轮缩放,而且没有参照。** `Shift+F` 打开的预览只有滚轮和 `R`,而 `R` 是按窗口的
  九成适应 —— 于是预览最该干的事,看像素,除了靠滚轮试之外没法定到 1:1。现在 `4` 是一个源像素
  对一个屏幕物理像素,`6` 是一个源像素对一个窗口逻辑点。两者差一个背景缓冲倍率,所以 Retina 上
  `4` 是 `6` 的一半,1x 显示器上两个键等价。含义与主界面上这两个键一致。`R` 仍是适应窗口,
  提示行里的说法从 Reset 改成了 Fit —— 它一直就是这个行为。
- **当前倍率显示在左上角**,滚轮缩放时有个数可落。刻意没有放进底部提示行:那行居中渲染,
  窗口一窄就从两端裁掉,唯一会变的那部分正好跟着一起没了。

### v1.4 — 2026-08-05

**4K 不再是慢放。**

- **4K 59.94fps 的比较实际只跑到约一半速度。** 帧送上屏幕前要转成 RGB,而这步转换只用一个核。
  按对齐输入分辨率所用的高质量参数,一帧 3840x2160 要约 31ms,而 59.94fps 每帧只有 16.7ms,
  于是播放就停在转换器跟得上的速度 —— 约 31fps,标称速度下就是慢放。1080p 只要 10.4ms、塞得进预算,
  所以只有 4K 会暴露出来。libswscale 能把这份活摊到多核上,但**只在 `sws_scale_frame()` 下生效**;
  `sws_scale()` 会直接忽略传给它的线程数。现在每个视频平分机器的核心。
  10 核 M4 实测:4K 一对 31.2 → 59fps,1080p/4K 混合一对 25.3 → 51fps,1080p 不变,仍是 60.0。
  画面不受影响 —— 任意线程数下都与单线程结果逐字节一致,包括升采样、降采样、奇数宽度这些
  切片边界本可能留下接缝的路径。
...
**裁剪预览** — `Shift+F` 选区后打开预览而非直接保存。可绕光标缩放、平移,`4` / `6` / `R` 分别跳到 1:1 物理像素、100% 和适应窗口,当前倍率显示在底部提示条。`Enter` 存拼接图,`Shift+Enter` 存左视频 + 每个右视频 + 拼接图。保存前用原生对话框选目录,文件名带时间戳前缀。
...
| 位置 | 问题 | 为什么还留着 |
|---|---|---|
| `video_compare.cpp:966`(`partial_seek_right_video`) | 局部 seek 可能在 demuxer 线程正处于 `av_read_frame` 时调用 `av_seek_frame`(同一个 `AVFormatContext`),属未定义行为。完整 seek 路径有握手机制,局部的没有。 | 需要慢速 I/O(网络流、高负载机器)才撞得上竞态窗口,无法按需复现。 |
| `video_compare.cpp:1373`(play-resume 重对齐) | 恢复播放时的重对齐若取不到帧,失败被丢弃,画面静止且无任何提示。 | 需要 seek 目标越过文件末尾,而常规 seek 路径通常先一步拦下。复现不出来。 |
| `video_compare.cpp:976`(`partial_seek_right_video`) | 时移乘数不为 1 时,局部 seek 的目标漏算了乘数自身的贡献。完整 seek 路径算了。 | 需要带乘数的 `--time-shift`,未测试。 |
| `video_compare.cpp:1237`(前进步进) | 前进步进触发 seek 后遇到文件末尾,会报"reached end of video",但偏移其实已经变了。 | 纯提示问题。 |
| `video_compare.cpp:1751`(`update_frame_timing`) | 步进仍会往帧时长均值里灌入不是帧时长的样本,`delta_pts_` 可能偏离真实值。 | v1.2 起偏移量已不依赖它。剩余影响只到同步容差和帧上标注的时长。 |
| `display.cpp:323`(`Display::Display`) | 加 `-d` 时无条件把窗口尺寸除以 2,假定显示器是 2x 背景缓冲。在 1x 显示器上(如 macOS 的 4K 原生档)没有 2x 补回来,drawable 直接减半 —— `-d` 反而让画质更差。 | 上游代码。正确做法是建窗后按实测的 `drawable ÷ 窗口` 倍率再定,但要改动窗口初始化顺序,且无法在各种显示器配置下逐一验证。绕过:1x 档位下不加 `-d`。 |
| `video_compare.cpp:1645`(`pop_frame`) | 步退后按 `Shift+D`,帧从该步退留存的缓冲里取。推断正确,实际使用也符合预期。 | 无日志证据 —— 调试打印无法把它和普通播放区分开。 |
...
- 帧以 RGB24 送上屏幕,而 Metal 不支持 24 位纹理,SDL 只能先在 CPU 上转成 ARGB8888 —— 4K 每帧约 3ms,是 v1.4 之后主线程上最大的单项开销。两个**分辨率不同**的 4K 视频对比仍差一点到 59.94fps,原因和转换本身昂贵是同一个:较小的一侧要先放大到较大的那一侧,然后才画。
