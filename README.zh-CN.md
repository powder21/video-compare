...
**本节只适用于 macOS 的 Retina / 缩放显示模式。** Windows 和 Linux 的 DPI 处理机制不同,
下面的实测数据和结论都不适用于它们。

比编码质量时,画面**被静默缩放过一次**你是看不出来的 —— 而缩放正好会抹掉你要找的高频细节,
或者自己造出像瑕疵的东西。以下是在一台 **4K 面板 + macOS** 上对 **2560×1440 视频**的实测结果。
...
**两个都到位才是真正的像素对像素。** 只加 `-d` 而不按 `4`,视频仍会被拉伸去铺满窗口;
只按 `4` 而不加 `-d`,1:1 是相对于逻辑像素的,面板上仍被放大一倍。

### macOS 缩放档位:1080p 反而最好

macOS 对非原生分辨率的档位,是**按逻辑分辨率的 2 倍渲染,再重采样到面板**。
所以在 4K 面板上:

| 档位 | 帧缓冲 | → 面板 3840×2160 |
|---|---|---|
| **1080p** | 1920×1080 × 2 = **3840×2160** | **正好相等,零重采样** ✓ |
| 1440p | 2560×1440 × 2 = **5120×2880** | 超出面板,**必须降采样 ×0.75** ✗ |
| **4K 原生** | 3840×2160(1x) | **正好相等,零重采样** ✓ |

**1080p 是 4K 面板上唯一"逻辑×2 正好等于面板"的档位。** 1440p 是唯一有损的 —— 这点很反直觉,
"更高的分辨率设置"反而更差。4K 原生同样无损,但系统 UI 会小到难以操作。

> 帧缓冲到面板这一步 SDL 看不到,上表是由实测的 `drawable ÷ 窗口` 倍率反推的:
> 1080p 和 1440p 档都报 2x,4K 原生档报 1x。
...
```sh
alias vc='video-compare -d -u'
```

打开后**按一次 `4`**。这样对任意 **≤4K** 的视频都是像素对像素:

| 视频 | 按 `4` 后 | 占屏 |
|---|---|---|
| 720p | 1:1 ✓ | 1280×720 居中 |
| 1080p | 1:1 ✓ | 1920×1080 居中 |
| 1440p | 1:1 ✓ | 2560×1440 居中(约 2/3) |
| 4K | 1:1 ✓ | 铺满 |

视频小于面板时画面就是小一块,四周留空 —— 那是物理必然,1:1 的 720p 在 4K 面板上本来就只有那么大。

### 用 `-v` 自查

```
Video size:            2560x1440
SDL GL drawable size:  ????x????
```

**`SDL GL drawable size` 是关键。** 它小于 `Video size` 就说明画面被降采样了;
按 `4` 之后 HUD 会显示当前倍率,可以对照。

### 几个坑

**`-u`(全屏)只适合视频分辨率 ≥ 面板的情况。** 它强制 drawable = 全屏,
2K 视频在 4K 面板上全屏会被放大 1.5 倍 —— 而默认是**最近邻**过滤(`Bilinear filtering: false`),
非整数倍的最近邻放大会产生不均匀像素块,**看起来很像编码瑕疵**。按 `4` 可以消除这个问题。

**`-d` 在 1x 档位(4K 原生)帮倒忙**,见下方已知问题表。那一档要 1:1 就别加 `-d`。

**1440p 档不要用于画质评估**,无论窗口还是全屏都有 ×0.75 的降采样。
